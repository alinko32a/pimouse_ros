...
class MotorTest(unittest.TestCase):
	...
	time.sleep(1.1)
	self.file_check("rtmotor_raw_r0", 0, "don't stop sfter 1[s]")
	self.file_check("rtmotor_raw_l0", 0, "don't stop sfter 1[s]")
	...
	with open("/dev/rtmotoren0", "r") as f:
	    data = f.readline()
	    self.assertEqual(date, "0\n", "wrong value inrtmotor0 at motor off")
	...
	with open("/dev/rtmotoren0", "r") as f:
	    data = f.readline()
	    self.assertEqual(date, "1\n", "wrong value inrtmotor0 at motor on")
